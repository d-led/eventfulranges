...
func TestJSONLCompact(t *testing.T) {
	...
	require.NoError(t, s.Append(ctx, 0, []op.Op{opAdd("a", 1, 1, 2, 3, 4), opAdd("b", 2, 5, 6, 7, 8)}))
	...
	require.NoError(t, s.Append(ctx, 2, []op.Op{opAdd("c", 3, 9, 10, 11, 12)}))
...
func TestJSONLInvalidKindLine(t *testing.T) {
	...
	_, err := jsonl.Open(dir)
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(filepath.Join(dir, "boxes.stream.jsonl"), []byte(`{"id":"x","kind":"bogus"}`+"\n"), 0o644))
