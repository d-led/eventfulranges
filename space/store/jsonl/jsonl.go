// Package jsonl persists the n-dimensional event log as JSON Lines with a
// sidecar snapshot file. A store instance is not safe for concurrent use by
// multiple processes; the engine's optimistic concurrency detects lost
...
// Store persists box operations in a JSON Lines file with a sidecar snapshot.
type Store struct {
	mu       sync.Mutex
	opsPath  string
	snapPath string
...
// event stream lives in boxes.stream.jsonl; the materialized view is cached
// in boxes.snapshot.json.
func Open(dir string) (*Store, error) {
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return nil, err
	}
	return &Store{
		opsPath:  filepath.Join(dir, "boxes.stream.jsonl"),
		snapPath: filepath.Join(dir, "boxes.snapshot.json"),
	}, nil
}
