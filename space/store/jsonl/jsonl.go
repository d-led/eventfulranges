// Package jsonl persists the n-dimensional event log as a single JSON Lines
// stream: operation records are appended in order and materialized snapshots
// are embedded as records in the same file, so the stream can be compacted.
// A store instance is not safe for concurrent use by multiple processes; the
