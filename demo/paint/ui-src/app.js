...
import { intervalToBoxes } from './morton.js';
...
// ---------- state ----------
let adds = []; // normalized [a,b) Morton intervals of painted cells
let removes = []; // normalized [a,b) Morton intervals of erased cells
let boxes = []; // materialized view: [{x, y, size}]
let logEntries = []; // the full operation log, for JSONL export
let clientID = '';
let clients = 0;
let total = 0;
let socket = null;
let pendingSeed = null; // boxes decoded from the ?d= share blob
...
// semantics (union of additions minus union of removals) and decodes the
// surviving Morton intervals back into cells.
function materialize() {
  boxes = [];
  for (const [a, b] of difference(adds, removes)) {
    boxes.push(...intervalToBoxes(a, b));
  }
  draw();
}
...
function applyEntries(entries) {
  ...
  for (const e of entries) {
    if (e.kind === 'add') newAdds.push([e.data.a, e.data.b]);
    else if (e.kind === 'remove') newRemoves.push([e.data.a, e.data.b]);
    logEntries.push(e);
    appendLog(e);
  }
...
function resizeCanvas() {
  ...
  if (boardEl.width !== Math.round(w * dpr) || boardEl.height !== Math.round(h * dpr)) {
...
function draw() {
  ...
  for (const b of boxes) {
    const sx = (b.x - cam.x) * px + w / 2;
    const sy = (b.y - cam.y) * px + h / 2;
    const sw = b.size * px;
    if (sx + sw < 0 || sx > w || sy + sw < 0 || sy > h) continue;
...
boardEl.addEventListener(
  ...
  (e) => {
    ...
    cam.scale = Math.min(64, Math.max(1, cam.scale * Math.exp(-e.deltaY * 0.0015)));
...
function commitStroke() {
  ...
  sendCmd({ kind: tool === 'erase' ? 'erase' : 'paint', data: { x0, y0, x1, y1 } });
}
...
function handleMessage(msg) {
  ...
  if (msg.type === 'state') {
    ...
    else if (!pendingSeed) materialize();
    maybeSeed();
  } else if (msg.type === 'op') {
...
function maybeSeed() {
  ...
  if (!seed) return;
  pendingSeed = null;
  if (logEntries.length > 0) return; // the session already has content
  for (const b of seed) {
    sendCmd({ kind: 'paint', data: { x0: b.x, y0: b.y, x1: b.x + b.size, y1: b.y + b.size } });
  }
}
...
async function refreshShareLink() {
  const raw = serializeBoxes(boxes);
  const rawBytes = new TextEncoder().encode(raw).length;
  const gz = await gzipBytes(raw);
  const encoded = encodeBase36(gz);
  const url = new URL(location.href);
  url.searchParams.set('d', encoded);
  shareEl.value = url.toString();
  const smaller = rawBytes === 0 ? 0 : Math.round((1 - gz.length / rawBytes) * 100);
  blobStatsEl.textContent = `${rawBytes} B → ${gz.length} B gzip → ${encoded.length} chars base 36 (${smaller}% smaller)`;
}

function downloadJSONL() {
  const lines = logEntries.map((e) =>
    JSON.stringify({ id: e.id, kind: e.kind, data: e.data, client: e.client, at: e.at }),
  );
...
copyShareBtn.addEventListener('click', async () => {
  try {
    const url = await refreshShareLink();
...
async function boot() {
  ...
  if (d) {
    try {
      pendingSeed = parseBoxes(await gunzipText(decodeBase36(d)));
      boxes = pendingSeed;
    } catch (e) {
      setStatus(`bad share blob: ${e.message}`);
    }
  }
